...
void Infill::_generate(Polygons& result_polygons, Polygons& result_lines, const SierpinskiFillProvider* cross_fill_provider, const SliceMeshStorage* mesh)
{
    ...
    case EFillMethod::CONCENTRIC_ARC:
        generateConcentricArcInfill(in_outline.offset(outline_offset), result_lines, line_distance);
    default:
...
void Infill::generateConcentricArcInfill(const Polygons& in_outline, Polygons& result_lines, const int line_distance)
{
    if (line_distance == 0)
    {
        return;
    ...
    int arc_count = sqrt(pow(std::max(abs(boundary.max.X), abs(boundary.min.X)), 2) + pow(std::max(abs(boundary.max.Y), abs(boundary.min.Y)), 2)) / line_distance;
    ...
    addArcInfill(outline, result_lines, line_distance, boundary, cut_list);
}

void Infill::addArcInfill(const Polygons& outline, Polygons& result, const int arc_distance, const AABB boundary, std::vector<std::vector<double>>& cut_list)
{
    unsigned int scanarc_idx = 0;
    int max_radius = sqrt(pow(std::max(abs(boundary.max.X), abs(boundary.min.X)), 2) + pow(std::max(abs(boundary.max.Y), abs(boundary.min.Y)), 2));
